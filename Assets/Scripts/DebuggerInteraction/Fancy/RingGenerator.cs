...
{
    static Vector3 min = new Vector3(0.75f, 0.6f, -2f), max = new Vector3(3f, 2f, 2.5f), centre = new Vector3((min.x + max.x) / 2f, 0, (min.z + max.z) / 2f);
    ...
    public static List<Vector3> Create(int size) //Returns list of positions
    {
        ...
        for (int i =0; i < size; i++)
        {
            ...
            positions.Add(new Vector3(Mathf.Sin(theta) * radius, 1.5f , Mathf.Cos(theta) * radius) + centre);
        }
        return positions;
    }
}
